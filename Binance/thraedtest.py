...
btc_balance = 0.004
while True:
    if datetime.datetime.now().time() > start_time and datetime.datetime.now().time() < end_time and datetime.datetime.today().weekday() < 5:
        for symbol in symbols:
            for time in times:
                marketprices = 'https://api.binance.com/api/v3/ticker/24hr?symbol=' + symbol
                res = requests.get(marketprices)
